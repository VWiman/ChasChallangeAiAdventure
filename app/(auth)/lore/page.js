...
export default function LorePage() {
	...
	const hometowns = [
		{
			...
			description: "A town perched high in the mountains, where the air carries whispers of adventure.",
		},
		{
			name: "Pineview",
			image: "./images/hometown/pineview.png",
			description: "A tranquil town surrounded by ancient forests, offering a peaceful sanctuary.",
		},
		{
			name: "Marinport",
			image: "./images/hometown/marinport.png",
			description: "A bustling harbor town known for its seafaring quests and vibrant trade.",
		},
		{
			name: "Eldorvik",
			image: "./images/hometown/eldorvik.png",
			description: "A town nestled among misty fjords, blending ancient magic with rugged natural beauty.",
		},
	];
	...
	function handleStartAdventure() {
		...
		if (!selectedRace) {
			setError("Please select your character's race before starting the adventure.");
			return;
		}
		if (!selectedClass) {
			setError("Please select your character's class before starting the adventure.");
			return;
		}
		if (!selectedHometown) {
			setError("Please select your character's hometown before starting the adventure.");
			return;
		}
		if (!name.trim()) {
			setError("Please enter your character's name before starting the adventure.");
			return;
		}
	...
	return (
		<>
			<section id="race" className="flex flex-col justify-center items-center gap-10 my-28">
				<h1 className="text-5xl font-normal text-center">Choose your character's race</h1>
			...
			<section id="class" className="flex flex-col justify-center items-center gap-10 my-28">
				<h1 className="text-5xl font-normal text-center">Choose your character's class</h1>
				<div className="grid gap-4 sm:grid-cols-2 sm:grid-rows-2 grid-cols-1 grid-rows-1">
					{classes.map((cls) => (
						<CardSelection
							key={cls.name}
			...
			<section id="town" className="flex flex-col justify-center items-center gap-10 my-28">
				<h1 className="text-5xl font-normal text-center">Choose your character's hometown</h1>
				<div className="grid gap-4 sm:grid-cols-2 sm:grid-rows-2 grid-cols-1 grid-rows-1">
					{hometowns.map((home) => (
						<CardSelection
							key={home.name}
			...
			<section className="flex flex-col justify-center items-center gap-5 my-28">
				...
				<p>Write your character name or generate one.</p>
				<div className="flex gap-2 max-w-xl w-full pb-12">
					...
					/>
					<Button onClick={() => handleRandomName()} size="rounded-xl h-16 px-3 w-[30%]">
						Generate
					</Button>
				</div>
				{error && (
				...
				<Button radius="rm" size="h-16 large sm:w-80 w-60" onClick={() => handleStartAdventure()}>
					Start Adventure
				</Button>
			</section>
		</>
