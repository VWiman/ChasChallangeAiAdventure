...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    setDisplayHistory((prevDisplayHistory) => {
      ...
      return [
        ...newDisplayHistory,
        <p className="py-10 text-black/60">{message}</p>,
      ];
    });
  }, [fullUserHistory]);
  ...
  const handleSendSummary = async () => {
    ...
    try {
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            {
              role: "system",
              content: "You are a fantasy novel author. Summarize into a short-story. Your response should not include any formating.", 
            },
          ],
        }),
      });
      ...
      if (response.ok && data.choices && data.choices.length > 0 && data.choices[0].message.content) {
        const fullSummary = JSON.stringify(data.choices[0].message.content); // Assuming message is a string, not JSON
        const newFullSummary = fullSummary.slice(1,-1).replace(/\\n/g, "\n")
        setSummary(newFullSummary);
      } else {
        throw new Error(
          (data.error && data.error.message) || "Failed to fetch response from OpenAI"
        );
      }
    } catch (error) {
  ...
  return response != "" ? (
    <div>
      ...
      <pre className="p-10 my-10 whitespace-pre-wrap text-slate-800 bg-stone-100 text-lg leading-relaxed">{summary}</pre>
    </div>
  ) : (
    <div>Loading...</div>
  );
